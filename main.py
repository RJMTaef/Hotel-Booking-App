...
class SecureCreditCard(CreditCard):
    def authenticate(self, given_password):
        ...
        # Check if there's exactly one matching password
        if password.shape[0] == 1:
            # Extract the single value from the Series
            password_value = password.iloc[0]
            if password_value == given_password:
                return True
            else:
                return False
        else:
            # Handle the case where there are no matches or multiple matches
            print("Error: Multiple or no passwords found for this card number.")
